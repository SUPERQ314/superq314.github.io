...
(function() {
    ...
    if (!window.requestAnimationFrame) {
        ...
        window.requestAnimationFrame = function(callback) {
            ...
            var id = window.setTimeout(function() { callback(curTime + timeToCall); },
                timeToCall);
...
function Game(id, params) {
    ...
    var $canvas = document.getElementById(id); //获取绘图区域
    $canvas.width = _.width;
    $canvas.height = _.height;
    var _context = $canvas.getContext('2d'); //获取CanvasRenderingContent2D对象
    //var gl = $canvas.getContext('webgl'); 获取3D对象
    var _stages = []; //布景对象队列
    var _events = {}; //事件集合
    var _index = 0, //当前布景索引
        _hander; //帧动画控制
